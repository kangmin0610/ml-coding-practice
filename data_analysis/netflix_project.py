...
# map 함수를 이용하여 rating 컬럼의 값을 딕셔너리 기반으로 변환하여 age_group 컬럼에 저장
# .map( ) : 사전에 정의한 내용을 변수로 적용
...
# 예시) 인덱스 1으 listed_in 열 값 : International TV Shows, TV Dramas, TV Mysteries
genres = netflix['listed_in'].str.split(', ', expand=True).stack().value_counts()
genres
...
ptl.title('Here we see interesting differences between countries. Most shows in South Korea are targeted to adults, for instance.',
          )