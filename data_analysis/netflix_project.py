...
# map 함수를 이요하여 rating 컬럼의 값을 딕셔너리를 기반으로 변환하여 age_group 컬럼에 저장
#  .map( ) : 사전에 정의한 내용을 변수에 적용 
netflix['age_group'] = netflix['age_group'].map(age_group_dic)
netflix.head(2)

# 데이터 전처리 완료한 데이터셋 csv파일로 저장 
...
netflix_age_country_unstack = netflix_age_country_unstack.div(netflix_age_country.sum(axis=0), axis=1)
netflix_age_country_unstack
...
text = np.array(Image.open('netflix_logo.jpg'))
